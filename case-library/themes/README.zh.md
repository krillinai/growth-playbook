# 主题案例

按照增长基础、生命周期环节与增长系统组织案例。公司页保留详细证据和来源背景，主题页比较不同公司如何处理同一增长问题。

**[English](README.md)｜[简体中文](README.zh.md)**

## 增长基础

| 主题 | 核心问题 |
| --- | --- |
| [产品市场匹配](product-market-fit/README.zh.md) | 哪些证据表明明确市场能够反复获得可行产品价值？ |
| [客户分群与 ICP（理想客户画像）](customer-segmentation/README.zh.md) | 哪些客户差异应该改变产品与增长决策？ |
| [定位](positioning/README.zh.md) | 哪种竞争语境能让最佳客户理解差异化价值？ |
| [漏斗与约束模型](funnel-and-constraint/README.zh.md) | 当前哪个转化环节限制增长系统，哪些证据可以把它与相邻症状区分开？ |
| [增长模型](growth-model/README.zh.md) | 价值、获客、留存、商业化、扩张与再投资如何随时间相互作用？ |

## 增长环节

| 主题 | 核心问题 |
| --- | --- |
| [获客](acquisition/README.zh.md) | 产品如何触达合格需求？ |
| [激活](activation/README.zh.md) | 新用户如何获得有意义的价值？ |
| [留存](retention/README.zh.md) | 用户为什么回来并加深使用？ |
| [商业化](monetization/README.zh.md) | 已交付价值如何转化成可持续收入？ |
| [推荐与扩张](referral-and-loops/README.zh.md) | 已交付价值如何创造新分发或更深采用？ |

## 增长系统

| 主题 | 核心问题 |
| --- | --- |
| [指标与衡量](metrics-and-measurement/README.zh.md) | 如何衡量价值、行为和经济性？ |
| [实验系统](experimentation/README.zh.md) | 团队如何把不确定性转化成决策？ |
| [增长基础设施](growth-infrastructure/README.zh.md) | 哪些增长能力应该成为可复用系统？ |
| [激励系统](incentive-growth/README.zh.md) | 奖励何时能改变行为而不破坏质量？ |
| [增长组织](growth-organization/README.zh.md) | 人员、责任与决策权应如何支持增长？ |

## 持续更新规则

1. 先把公司专属证据加入公司页。
2. 按增长基础、生命周期环节与增长系统给证据分类。
3. 只更新真正相关的主题对照页。
4. 只有新证据实质性支持或挑战现有模式时，才修改跨公司结论。
5. 保留来源日期、证据类型、参与者视角和适用边界。
6. 把发布方公布的增长、ARR（年度经常性收入）、估值、客户数与 DAU（日活跃用户）主张同因果结论分开。
